...
public class Conversation : MonoBehaviour
{
    ...
    private static List<dialogue[]> conversations = new List<dialogue[]> {
        ...
        new dialogue[] {
            ...
            new dialogue("Oh it's easy! All you have to do is walk through one of the doors!", "npc", "")
        },
        new dialogue[] {
            new dialogue("Hi", "player", ""), 
        ...
        new dialogue[] {
            ...
            new dialogue("Well it's the only place I want to go really. I honestly don't know anywhere else someone could go to.", "npc", ""),
        },
        ...
        new dialogue[] {
            ...
            new dialogue("That house? That's the house everyone is supposed to be in.", "npc", ""),
    ...
    private static dialogue[] last_conversation = new dialogue[] {
        ...
        new dialogue ("Are you ok?", "npc", "", 1.5f),
        ...
        new dialogue ("Here come with me I’ll show you the doors.", "npc", ""), 
        ...
        new dialogue ("Are you sure you want to see them? Maybe that's why they're not there.", "npc", "rotateplayer", 0.3f),
        new dialogue ("But how would that make sense doors don’t just appear out of thin air they are always there.", "player", ""),
        new dialogue ("I’m sorry I don’t have all the answers.", "npc", ""),
        ...
        new dialogue ("I’m really sorry, do you want me to stay with you until you see one? I can stay with you until you see one.", "npc", ""),
    ...
    private static dialogue[] overman_conversation = new dialogue[] {
        ...
        new dialogue ("I tried pretty hard to get in.", "player", ""),
        new dialogue ("That's alright.", "npc", ""),
        new dialogue ("Does anybody else ever come into this house?", "player", ""),
        new dialogue ("It's only really ever been you that comes in every once in a while.", "npc", ""),
        ...
        new dialogue ("Plus if I left it'd get too dirty, I've gotta keep it clean just in case you come by.", "npc", "", 2.7f),
        new dialogue ("Just like now.", "npc", "", 3.0f),
        ...
        new dialogue ("Sure. I can't guarantee that I'll have answers though.", "npc", ""),
        ...
        new dialogue ("I guess it just seems like I can't actually love myself and others if I'm not in that house.", "player", ""), 
        new dialogue ("How can I ever be justified if I'm not in that house? How can I ever be truly alright if I don't ever find a way in?", "player", ""),
        new dialogue ("I try not to think about it but I always do. I always think about the house. I look for other houses but I feel wrong, I've been hearing about that house all my life and I feel wrong that I'm not in it.", "player", ""),  
        new dialogue ("Why do you feel wrong?", "npc", ""),
        new dialogue ("I don't know I've just always heard that house was supposed to be the purpose of all this walking.", "player", ""),
        new dialogue ("Always hearing that it's the right way to go, nothing else will satisfy. No matter how hard I try subconciously I'm always thinking like that.", "player", ""), 
        new dialogue ("But I can't get in.", "player", ""),
        ...
        new dialogue ("But still I need something to hold onto. Out in the sand it's too all over the place I slip up too much the weather is too volatile.", "player", ""),
        new dialogue ("And people out there keep telling me I just need to go into that house and things will be better", "player", ""),
        ...
        new dialogue ("", "npc", "", 3.0f),
        new dialogue ("What if I did truly do this to myself because I wanted to though, what if I'm dooming myself because it feels artistic.", "player", ""),
        new dialogue ("What if I really am keeping myself from there because I subconciously romanticize the idea of sitting in the sand, and getting stuck in storms.", "player", ""),
        new dialogue ("And what if that's so ingrained in my head that I just become irreversibly fucked? Eternally fucked?", "player", ""),
        new dialogue ("What if I eventually become stuck in the sand because I consistently put myself back there, and then I can't get out?", "player", ""),
        new dialogue ("And by that point it's too late?", "player", ""),
        new dialogue ("I don't think it will be too late.", "npc", ""),
        new dialogue ("I've tried I tried really hard you know I really tried to get into that house.", "player", ""),
        new dialogue ("I know you did. I'm sorry.", "npc", ""),
        new dialogue ("I just want to know if everything will be ok.", "player", ""),
        new dialogue ("I know you do.", "npc", ""),
        new dialogue ("So then what do I do? Where do I go?", "player", ""),
        new dialogue ("Keep trying to get into that house even though I don't believe in it but I'm anxious I should?", "player", ""),
        new dialogue ("Or resign myself to sitting in the sand forever because that feels poetic?", "player", ""),
        new dialogue ("I think you can just keep walking.", "npc", ""),
        ...
        new dialogue ("", "npc", "startstains", 3.0f),
        new dialogue ("Yeah.", "player", ""),
        new dialogue ("", "npc", "despawnhouse", 15.0f)
    };
    ...
    public static bool npc_gui_fading;
    bool player_cant_click_again;
    ...
    void Start()
    {
        ...
        GameObject.FindGameObjectWithTag("house").GetComponent<NavMeshObstacle>().enabled = false;
        ...
        npc_gui_fading = false;
        player_cant_click_again = false;
    }
    ...
    void Update()
    {
        ...
        if (active_conversation && conversation_index > 0 && !doing_action && !revealing_text && !player_cant_click_again) {
    ...
    private void InitializeConversation(Dictionary<string, object> message) {
        ...
        if ((string) message["npc"] == "last_npc" && !last_conversation_over) {
            ...
            if (conversations.Count == 1 && (string) message["gender"] == "female") {
                int filler_convo = Random.Range(0, 3);
                if (filler_convo == 0) {
                    dialogue[] convo = {new dialogue("Sorry, can't talk.", "npc", "")};
                    conversations.Insert(0, convo);
                } else if (filler_convo == 1) {
                    dialogue[] convo = {new dialogue("So sorry, I'm busy right now.", "npc", "")};
                    conversations.Insert(0, convo);
                } else if (filler_convo == 2) {
                    dialogue[] convo = {new dialogue("No time to talk, sorry.", "npc", "")};
                    conversations.Insert(0, convo);
                }
            } else if (conversations.Count == 1 && (string) message["gender"] == "male") {
                EventManager.TriggerEvent("StopSpawningNPCs", null);
            }
    ...
    IEnumerator RevealInitialText(dialogue text, TMP_Text text_object) {
        ...
        if (text.speaker == "npc") {
            if (!offscreen) {
                ...
                StartCoroutine(LerpOffscreenAlpha(.97f, .1f, offscreen_image));
            }
        } else {
            StartCoroutine(LerpOffscreenAlpha(1f, .1f, player_gui));
        }
    ...
    IEnumerator RevealPlayerText(dialogue text) {
        ...
        if (current_conversation[conversation_index].action.Length > 0) {
            Debug.Log("action event" + current_conversation[conversation_index].action );
            doing_action = true;
            EventManager.TriggerEvent("NpcAction", new Dictionary<string, object> {
    ...
    void TextOffscreenListener(Dictionary<string, object> message) {
        ...
        if (offscreen) {
            ...
            if (offscreen_text.text.Length > 0) {
                StartCoroutine(LerpOffscreenAlpha(.97f, 0.1f, offscreen_image));
            }
